...
const RadioGroupComp = ({ className, options, optionClass, label, disabled = false, ...props }) => (
  <>
      ...
      <div className='mt-3 space-y-3'>
        {options.map((option) => (
          <div key={option.value} className='flex items-center'>
            ...
            <label htmlFor={option.value} className='ml-2 block text-sm font-medium text-gray-700 dark:text-gray-400'>
              {option.label}
            </label>
          </div>
        ))}
