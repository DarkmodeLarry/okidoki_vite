...
import { usePostQuery } from '@/libs/apiHooks'
import { UPLOAD_MEDIA_BASE_URL } from '@/libs/baseUrl'
import { cn } from '@/libs/utils'
import { Eye, XCircle } from 'lucide-react'
import React, { useRef } from 'react'

const Upload = React.forwardRef(
  ({ className = {}, setValue, onChange, onMediaChange = null, name, label = 'Upload', ...props }, ref) => {
      ...
      <div className={cn(className.wrapper)} ref={ref}>
        {props.value ? (
          <div className='flex items-center'>
            <a className={cn('flex items-center')} href={props.value} target='_blank' rel='noreferrer'>
              <Eye />
              <span className='ml-2'>View media</span>
            </a>
            <Button className='ml-3' variant='outline' onClick={() => setValue(name, '')}>
              <XCircle />
            </Button>
          </div>
        ) : (
          <div className='w-1/4'>
            <input
              ...
              onChange={(e) => {
                ...
                if (!file) return
                onMediaChange ? onMediaChange(onChange, file) : handleMediaUpload(file)
              }}
              id='upload'
              ref={handleImportRef}
