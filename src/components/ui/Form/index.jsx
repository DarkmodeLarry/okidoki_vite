...
const Form = ({ children, rhfProps, formConfig, handleSubmit: handleSubmitFunc, wrapperClass }) => {
  ...
  const renderField = (field, control, error = <></>) => {
    ...
    switch (type) {
      ...
      default:
        return <Input className={className} disabled={disabled} type='text' {...register(name)} />
    }
  }
        ...
        formConfig.map((field) => (
          <ErrorBoundary>
            {renderField(
              ...
              control,
              errors?.[field.name] ? (
                <p className='mt-2 text-xs text-red-600'>{errors?.[field.name]?.message}</p>
              ) : (
                <></>
              )
            )}
