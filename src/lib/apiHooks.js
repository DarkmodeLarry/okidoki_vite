...
export const usePostQuery = (
  ...
  const rhf = useMutation(
    key,
    async (values) => {
      const res = await axios.post(customUrl ? customUrl(values) : url, values, {
    ...
    },
    {
      onError: (error) => {
        toast.error(
          error?.response?.data?.data?.error_details || error?.message || errorString,
          toastErrorOptions
        )
        onErrorContinued()
      },
...
export const useInfiniteGetQuery = (
  ...
  const rhf = useInfiniteQuery(
    key,
    async ({ pageParam = 1 }) => {
      const res = await axios.get(customUrl ? customUrl(pageParam) : url + `?page=${pageParam}`, {
        headers: prop.headers,
        ...axiosOptions
      })
      return res.data
